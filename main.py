...
def get_deals(): soup = BeautifulSoup(requests.get(AMAZON_URL, headers=USER_AGENT).text, "html.parser") deals = soup.select(".DealGridItem")[:POST_LIMIT] extracted_deals = []

print(f"[DEBUG] Found {len(deals)} potential grid items.")

for deal in deals:
    try:
        title_tag = deal.select_one(".DealTitle") or deal.select_one(".a-size-base-plus")
